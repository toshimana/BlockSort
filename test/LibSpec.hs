...
spec = do
    describe "solve" $ do
        it "sample09" $ do
            ...
            calcTargetRoot (StartPoint 10) (EndPoint 11) bns `shouldBe` [(BonusPoint 23,Cost 1201.118,[17,10,40,12,42,13,43,14,43,13,37,8,30,5,22,1,19,23,24,6,24,2,24,31,8,31,32,25,3,25,32,9,32,33,7,27,4,27,7,33,39,38,43,13,43,38,44,15,41,11,18],array (Red,Black) [(Red,8),(Green,6),(Blue,9),(Yellow,13),(Black,14)])]
            
        it "sample11" $ do
            let ic = toInitCode $ array (Red,Black) [(Red,2),(Green,1),(Blue,3),(Yellow,4),(Black,10)]
            createRootFromCode 1 4000 ic `shouldBe` []
